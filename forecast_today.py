...
for url in range(len(url_list)):
    ...
    high = forecast['data'][0]['max_temp']
    high_temp.append(round(high))
    low = forecast['data'][1]['min_temp']
    low_temp.append(round(low))    
    pop.append(forecast['data'][0]['pop'])
    daily_precip.append(forecast['data'][0]['precip'])
    weather.append((forecast['data'][0]['weather']['description']).lower())
    high2.append(round(forecast['data'][1]['max_temp']))
    high3.append(round(forecast['data'][2]['max_temp']))
    weather2.append((forecast['data'][1]['weather']['description']).lower())
    weather3.append((forecast['data'][2]['weather']['description']).lower())
    apparent_T.append(round(forecast['data'][0]['app_min_temp']))
